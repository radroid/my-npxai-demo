...
> 🟡 **The eval battery is no longer blocked on the pause** — as of 2026-07-14 it can run against **local Supabase** (`supabase/LOCAL.md`). The items below now gate the *deployed demo*, not the eval numbers. Full recipes in `docs/orchestration/manual-verification.md`.

- [ ] **Un-pause the Supabase project** (`NPX-demo`, ref `ptepxophdneugvcziqny`). Confirmed via `supabase projects list` that it still exists and is linked — it is paused, not deleted (DNS returns NXDOMAIN, same signature as the 2026-05-14 pause). The CLI has no unpause command, so this needs the dashboard — or export `SUPABASE_ACCESS_TOKEN` in an agent session and the Management API restore endpoint can do it.
  - ⚠️ **No longer blocks the evals** — run those against local instead (`bun run db:local`, then re-ingest). Still blocks the **public demo URL**: Cloudflare's edge cannot reach a DB on the Mac mini, so the deployed Worker needs the hosted project alive.
  - Once un-paused, do two things: (1) ship `20260714000000_fix_handle_new_user_search_path.sql` + `20260714010000_service_role_statement_timeout.sql` (note: this migration now ends in `NOTIFY pgrst, 'reload config';` — without it, hosted's long-lived PostgREST would keep enforcing the old 8s `service_role` timeout even after `db push` "applies" the migration; see `supabase/LOCAL.md` § Troubleshooting for how to verify the *effective* timeout, not just `pg_roles.rolconfig`) + `20260714020000_regdoc_chunks_staging_swap.sql` (non-destructive-until-safe re-ingest — see `supabase/LOCAL.md` § Re-ingest the corpus) — **new signups on hosted are almost certainly 500ing today** (the trigger lost its `search_path`; existing users are unaffected, which is why it went unnoticed). **Do NOT run a bare `bunx supabase db push`** — it aborts with "Found local migration files to be inserted before the last migration on remote database" because the five reconstructed foundational migrations (`20260416120*`) have no row in hosted's `schema_migrations` and sort before hosted's recorded history. **Never run the `--include-all` flag the CLI suggests either** — it would replay those five reconstructed migrations against the live hosted DB (DROP/CREATE POLICY, REVOKE ALL, CREATE INDEX hnsw, CREATE OR REPLACE on the RPCs), silently overwriting whatever's actually there. Correct sequence, see `supabase/LOCAL.md` § Migrations: `bunx supabase migration repair --status applied --linked 20260416120000 20260416120100 20260416120200 20260416120300 20260416120400` (marks them applied without executing them), then `bunx supabase db push --linked`; (2) deploy the daily keep-alive Worker so this is the last pause.
...
- [x] item-2 · RAG eval framework — golden dataset from ingested corpus; metrics module (faithfulness, answer relevancy, context precision, context recall, consistency — web-researched definitions, cited); experiment runner with structured JSONL logs + per-run cost guard; run experiments; commit scored report with realistic percentages per category. *(framework + report committed in prior sessions; hardened + extended in PR #13 below.)*
- [x] item-2b · **RAG eval hardening + embedding upgrade → PR #13 (MERGED 2026-07-14).** Hardened the test set (golden 65→92, OOC probes 23→37 across 4 new adversarial categories, anti-echo phrasing, retrieval-reachability metric) and upgraded embeddings to `text-embedding-3-large`@3072 via halfvec — a measured win (hit@8 95.7→96.7%, recall@8 82.6→86.0%, MRR 0.761→0.808). Rejected a reranker stage (regressed). Deployed to HOSTED under explicit authorization: foundational `migration repair` → `db push` (4 July migrations incl. signup-500 fix) → re-ingest at -large (1945 rows, 0 nulls) → merge. **Post-deploy verification caught + fixed an adversarial regression** (prompt rule 2d, commit a34de92): -large's stronger retrieval made ooc-017 misattribute a requirement to a doc we don't have; negative rejection dipped 100→97.3% then restored to 37/37 with the fix, verified no golden over-rejection. Known pre-existing: s002/s023 (section-specific golden Qs) refuse when the exact gold chunk misses the top-8 (~3% hit@8 miss; not a regression). ✅ Deployed-Worker smoke passed on https://npx.curlycloud.dev: in-corpus "shift turnover" → cited answer (REGDOC-3.6/2.3.4/3.1.1); OOC "REGDOC-2.4.2…" → correct refusal (rule 2d live). Window closed; prod fully on -large.
...
- [x] pre-item · **Local Supabase** — full stack in Docker on the Mac mini, unblocking the eval battery from the paused hosted project. Runbook `supabase/LOCAL.md`; `db:local*` scripts in `package.json`; LOCAL block in `.env.example` (`.env.local` deliberately untouched — it holds real secrets). Verified: 16 migrations replay clean, `service_role` statement_timeout raised via migration `20260714010000_service_role_statement_timeout.sql` (moved out of `seed.sql` so it also reaches hosted via `db push` — ingest was intermittently timing out at 8s and half-populating the table), 1,945 chunks / 19 docs re-ingested, anon-key vector search returns correctly-ranked rows over PostgREST, magic-link auth + Inbucket + tier classification all working (2026-07-14).
