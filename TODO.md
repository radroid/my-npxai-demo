...
- [~] item-2b · **RAG eval hardening + embedding upgrade → PR #13** (opened 2026-07-14). Hardened the test set (golden 65→92, OOC probes 23→37 across 4 new adversarial categories, anti-echo phrasing, retrieval-reachability metric) and upgraded embeddings to `text-embedding-3-large`@3072 via halfvec — a measured win (LOCAL, real OpenAI: hit@8 95.7→96.7%, recall@8 82.6→86.0%, MRR 0.761→0.808). Rejected a reranker stage (regressed). Full gate green; adversarial review = migration-safety/cost-wallet APPROVE, deploy-ordering REVISE (resolved operationally). **PR is HELD — merge only after the hosted migration+re-ingest human task above (in-place swap has an unavoidable outage window; code must not deploy ahead of the DB).**
