...
**Phase 5 — Pre-deploy** (Monday April 20) — **agent-side ship-ready 2026-04-17 night**

Overnight run (2026-04-17 → 18) closed Phase 3 + 4 + 5 agent scope. Deployment + Loom recording remain — those are Raj's gates. Current state of `main`:

- **Phase 3 RAG.** Real `/api/knowledge-hub/query` with `withGuard` (rate limit + circuit breaker + opt-in `x-eval-bypass` for the eval runner), `text-embedding-3-small`, `match_regdoc_chunks` RPC, D.3 fallback (`LOW_SIM_OOS = 0.40`, `MIN_CHUNK_SIM = 0.35` filter, avg-disclaimer at 0.35), `stripHtmlTags` + `sanitizeQueryText` on user input (strips tag markup + JS function-call leftovers), D.2 context envelope, `gpt-4o-mini` streamed through `StreamingGuard`, and a `data-sources` UIMessage part for the Sources panel. 30-min Redis response cache on successful LLM answers — 50× speedup on repeat queries. `bun run eval:kb` **20/20, all adversarial pass**.
- **Phase 3 tests.** `bun run test:tiers` runs 4 guard assertions (1400-char → 400, empty → 400, 4th-minute → 429, per-IP bucket isolation) using synthetic `x-forwarded-for`. Signed-in + npx_circle tiers documented as manual-via-sign-in-modal.
- **Phase 4 Generator.** `/api/generator/turnover` wraps `withGuard`, validates enum via `generatorInputSchema`, pulls `get_turnover_snapshot(p_unit)` RPC, streams `gpt-4o-mini` with `GENERATOR_SYSTEM` + `max_tokens = 1500`, runs output through `scanOutput`. Frontend at `/generator` has Station/Unit/Shift selects (defaults to the F.5 money-shot Unit 3 Evening), four component states per G.4, `react-markdown` rendering with `[CRITICAL]`/`[ATTENTION]`/`[ROUTINE]` badge parsing, Copy-Markdown button. Seed-augment migration applied — 52 plant_status / 15 work_orders / 24 shift_log_entries.
- **Phase 4 frontend.** Sources panel wired via `makeAssistantDataUI({ name: "sources" })` — inline chunks list with REGDOC badges (requirement blue / guidance amber), section number/title, similarity score, snippet, CNSC deep-link. Citation chips via `p`/`li` override in `markdown-text.tsx` matching the D.5 regex — `[REGDOC-X.X.X §Y.Z]` becomes a pill with `data-citation="true"`. Empty-state starter questions (Appendix E Q1–5) click-to-send via `aui.composer().setText + send`. `role="log" aria-live="polite"` on the thread viewport.
- **Phase 4 site.** Homepage — hero + 4-card showcase + "Why NPX AI?" pillars (20/20 eval score called out) + UI-only newsletter. Insights + Equivalency concept explainers. FAQ page (10 Qs). Sidebar-behind-navbar bug fixed.
- **Phase 5 audits.** Zero hex in `app/` + `components/`. OpenGraph + Twitter card metadata. `bun run preflight` clean. `bun run build` clean (13 routes). Redis response cache. All four G.4 states on Generator + KH.
- **UI verified via Browserbase** (2026-04-17 night): starter click submits; `[REGDOC-2.3.4 §3.2.3]` renders as chip; Sources panel expands to 8 chunks (top sim 0.652); nav/sidebar positioning fixed; mobile 375px no horizontal scroll.

What's left for Raj: (a) record the Loom per Appendix I.1, (b) flip the GitHub repo to public, (c) push main to trigger the Cloudflare deploy, (d) post-deploy E2E smoke (sign-in round-trip, anon 6th-query 429, eval:kb against prod URL).
