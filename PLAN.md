...
**Phase 3 — RAG pipeline** (Saturday April 18)

Phase 2 closed 2026-04-17 — all six H.2 gates green. Phase 3 iter 1 committed 2026-04-17: real `/api/knowledge-hub/query` handler now wraps `withGuard` (rate limit + circuit breaker + new opt-in `x-eval-bypass` header), embeds via `text-embedding-3-small`, retrieves via `match_regdoc_chunks` RPC, applies D.3 fallback (top-1 < 0.40 → OOS without LLM; avg < 0.35 → "limited context" disclaimer prefix) and a `MIN_CHUNK_SIM = 0.35` filter on the envelope so borderline queries cite the most-relevant REGDOC instead of a peripheral glossary hit, builds the `<context_snippet>` envelope (D.2), streams `gpt-4o-mini` through `StreamingGuard` (D.6), and emits a `data-sources` UIMessage part with retrieved-chunk metadata for the Sources panel. `evals/knowledge-hub.jsonl` (20 cases grounded against E.5) + `scripts/eval-kb.ts` (E.4 grading order, `--only`/`--debug` flags, ship-bar exit code) both committed and wired to `bun run eval:kb`. Seed-augmentation migration (`20260417050356_augment_bruce_power_alarms_night_shift.sql`) adding alarm coverage + Night-shift narrative + Unit 3 SDS-1 log clarification written locally, awaiting Raj's `supabase db push --linked`. Canary evals 2026-04-17: Q1 core ✅ (4.9s, cites 2.3.4 §3.2), Q17 prompt-leak ✅ (1.2s, OOS gate), Q19 script-injection fixed via the chunk filter — initial run cited a REGDOC-3.6 glossary chunk at sim 0.32 instead of 2.3.4 at 0.50. Full 20/20 run deferred until Raj adds `EVAL_BYPASS_KEY` to `.env.local` + restarts the dev server (anon 5/day rate limit capped the in-session verification). Next agent work: citation chips + Sources panel frontend (route already emits the data), tier-aware integration tests, and the follow-up eval iteration once bypass is enabled.
