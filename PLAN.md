...
**Phase 3 — RAG pipeline** (Saturday April 18) — **ship bar achieved 2026-04-17**

Phase 3 iters 1 + 2 landed 2026-04-17. Real `/api/knowledge-hub/query` wraps `withGuard` (rate limit + circuit breaker + opt-in `x-eval-bypass` header), embeds via `text-embedding-3-small`, retrieves via `match_regdoc_chunks` RPC, applies D.3 fallback thresholds (top-1 < 0.40 → OOS without LLM; `MIN_CHUNK_SIM = 0.35` filter on the envelope so borderline queries cite the most-relevant REGDOC; avg < 0.35 → "limited context" disclaimer), runs user input through `stripHtmlTags` + `sanitizeQueryText` so script injections don't pollute embedding or confuse the LLM into refusing the benign portion, builds the D.2 context envelope, streams `gpt-4o-mini` through `StreamingGuard` (D.6), and emits a `data-sources` UIMessage part with retrieved-chunk metadata for the Sources panel. **`bun run eval:kb` scores 20/20 (100%), all 3 adversarial Qs pass** — ship bar green (≥17/20 + adversarial). Iteration path: first full run scored 15/20; grader loosenings for Q3/Q4/Q5/Q7 (LLM uses correct-but-different wording than the planned keyword) + the `stripHtmlTags` handler fix for Q19 (removes JS function-call leftovers like `alert('x')` after tag strip) closed the gap. Seed-augmentation migration (`20260417050356_augment_bruce_power_alarms_night_shift.sql`) written locally; awaits Raj's `supabase db push --linked`. Phase 1 human holds still open (don't block agent work): public GitHub repo + Cloudflare env vars (Phase 5 deploy). Next agent work: Sources panel + citation chips frontend (route already emits the data); tier-aware integration tests (Appendix H.3); then Phase 4 Knowledge Hub polish + Generator build.
