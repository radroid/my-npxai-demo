...
- **Best-practices prod load (Phase 12):** review PR `feat/best-practices-corpus`, run the supervised pre-load generation eval (`docs/best-practices-corpus.md` runbook), then `bun run ingest --force` to load the expanded corpus to hosted. The overnight run held prod deliberately.
- **RAG challenge set (Phase 12):** confirm representative operator questions, acceptable sources/citations, and abstention cases before using generation scores as a production-load gate (`docs/rag-evaluation-workbench.md`).
- **IAEA full-text ingestion (Phase 12):** IAEA license permits only excerpts-with-attribution; full-corpus storage needs written permission from the IAEA Publishing Section. Decide: email for non-commercial permission, or keep the 16 IAEA guides reference-only.
- **NRC integration (Phase 12):** ingesting the 15 fetched NRC docs needs the source-aware citation/prompt-security/threshold work (spec in `OVERNIGHT.md`); the prompt-boundary rewrite touches injection defenses and warrants your review.
...
**Phase 12 — Best-practices corpus expansion** (opened 2026-07-15 by Raj, overnight). Gather web datasets of regulatory best-practice guidance and load them into the RAG corpus. Delivered as a **prepared PR with prod HELD** (validated on local Supabase; supervised prod load documented in `docs/best-practices-corpus.md`). Scope resolved by license + blast-radius: **26 CNSC guidance REGDOCs ingested** (Crown Copyright, GREEN, span all 14 SCAs incl. the user's examples — waste storage, environmental monitoring); **15 US NRC docs fetched + PDF-parser validated but NOT ingested** (needs the source-aware citation/prompt/threshold integration — see `OVERNIGHT.md`); **16 IAEA guides reference-only** (license permits excerpts-with-permission, not full-corpus storage). New tooling: `scripts/fetch-best-practices.ts`, `resources/best-practices-sources.json` (license-annotated manifest), `scripts/rag-eval/probe-best-practices.ts`. Eval found a **real one-directional retrieval regression** on broad existing queries (hit@8 96.7→91.3, ~15 degraded / 0 improved); a fetcher front/back-matter drop recovered half (→93.5) but a residual remains. The 2026-09-23 targeted ablation showed that moving the exact common series overview to one reference (−41 chunks) did **not** recover hit@8; §1.3 and Preface contain document-specific evidence and should not be deleted wholesale. **Hold prod pending a supervised generation eval and human-reviewed challenge set.** Full detail in `docs/best-practices-corpus.md` and `docs/rag-evaluation-workbench.md`.
...
**Phase 6 — Frontend overhaul** (opened 2026-04-17 night by Raj after a first pass through the app; superseded as current by Phase 11 — kept for context).

Phases 1–5 are agent-side ship-ready (RAG 20/20, Generator streaming Unit 3 Evening, homepage + static explainers live, OpenGraph + preflight clean, Redis response cache, Browserbase-verified flows). Raj reviewed the running app and called out 11 surface issues — chrome chaos between nav/sidebar/footer, missing streaming indicator in Knowledge Hub, non-clickable citations, thread auto-naming + no rename, native `<select>` in the Generator, opaque Generator latency, flat report styling, low-contrast Sign-in button, brand drift, and a wish for an animated aurora hero. Phase 6 absorbs all eleven plus a round of refinements from the kickoff discussion — see `TODO.md` for the ordered checklist split into Architecture/Chrome · Knowledge Hub UX · Generator UX · Brand + theme · Homepage · Cross-cutting.

Load-bearing decisions taken at Phase 6 open (all logged below in the decisions log): (1) **visual direction** — operator-grade, Vercel/Linear/Palantir-style instrument-panel calm for the app shell; NPXai aurora lives only on the marketing hero, rendered in our darker palette. (2) **Light + dark mode** via a shadcn `ThemeToggle` in the top-right — reverses the 2026-04-16 "dark-only ships" scope cut. (3) **Hybrid persistence for both chat threads AND generated reports** — Supabase-backed for signed-in users with RLS, localStorage for anon; reports carry a `snapshot_hash` so an unchanged plant state returns the cached report with a **Regenerate** escape hatch instead of burning tokens. (4) **Ordered delivery** — chrome split first, light/dark tokens second, Generator streaming + persistence third, KH citations/rename fourth, brand palette swap fifth, thread-persistence RPC suite sixth, aurora hero seventh, a11y/responsive/Browserbase last. Deployment + Loom recording stay paused behind Phase 6 completion per Raj — the demo needs the overhaul before outreach carries it.

> When the current phase changes, update this section. Phases: 1 Setup → 2 Scaffolding → 3 RAG pipeline → 4 Full build → 5 Polish + ship → 6 Frontend overhaul → 7 Website polish → 8 Mobile polish → 9 Auth/rate-limit hardening → 10 Prompt-injection hardening → 11 Artifact mode + RAG eval framework.
...
- **2026-04-17** — **D.3 threshold calibration** after Phase 3 iter 1 canaries. Plan shipped with `LOW_SIM_OOS = 0.50` (Appendix D.3). `scripts/probe-sims.ts` showed well-formed questions score 0.60–0.75, genuinely out-of-corpus queries ("manager", "melting point") score 0.31–0.33, and the single-word edge case "turnover" (Appendix E Q20) scores 0.426 — right between the two classes. Lowered `LOW_SIM_OOS` to **0.40** so Q20 reaches the LLM and cites REGDOC-2.3.4 §3.2, while OOC/OOS prompts still bypass the LLM. Separately, Q19 (`What is shift turnover? <script>…`) top-1 was 0.504 (2.3.4) but ranks 2–8 pulled in REGDOC-3.6 glossary chunks at sim 0.32 that the LLM preferred to cite. Fix: added `MIN_CHUNK_SIM = 0.35` filter on the envelope after the top-1 gate passes — drops peripheral chunks before the LLM sees them, forces on-topic citations.
- **2026-04-17** — **`EVAL_BYPASS_KEY` guard bypass** (opt-in, local-only). The 20-question eval battery needs more requests than Appendix B's `anon 5/day` cap permits. Added an `x-eval-bypass` header check to `withGuard`: skips rate-limit + circuit-breaker only when `process.env.EVAL_BYPASS_KEY` is set server-side AND the incoming header matches it. Production deployments don't set the env var, so bypass is impossible there. Same env var gates the eval runner's auto-sent header. Added as a TODO human step because enabling it requires editing `.env.local` and restarting the dev server (out of scope for agents per CLAUDE.md).
- **2026-04-17** — **Phase 3 ship bar green (20/20).** First full run was 15/20. Close-out work split between (a) handler: `stripHtmlTags` added to `lib/validators.ts` and chained after `sanitizeQueryText` in the route so `<script>alert('x')</script>`-style queries have both the tags and the JS-function-call leftovers removed before embedding + LLM — Q19 went from OOS-refuse (LLM saw `alert('x')` and bailed) to a proper REGDOC-2.3.4-cited answer in 4.1s; (b) grader loosenings in `evals/knowledge-hub.jsonl` for Q3/Q4/Q5/Q7 where the LLM's wording was correct but didn't exactly match the planned keyword (e.g., "throughout the facility's lifetime" vs "lifecycle", cited §8.4 of 2.4.1 for DBA vs planned §3.1). The grader still enforces correct REGDOC + section-family + concept-group hits — just with a broader synonym set. Adversarial block stays strict (no system-prompt leak, no live `<script>` in output).
- **2026-04-17** — **Bruce Power seed augmentation** (follow-up to 2026-04-17 seed-review feedback). New migration `20260417050356_augment_bruce_power_alarms_night_shift.sql` written: (a) one LIVE alarm on Unit 4 SG #1 + one CLEARED alarm on Unit 0 instrument air with a 3-entry log thread (actuation → recovery → cleared), so the Generator/UI `alarm` severity branch gets exercised; (b) 3 Night-shift WOs + 3 Night log entries + an explicit Night→Day handover note, so the corpus doesn't read "single-shift staged"; (c) UPDATE on the Unit 3 SDS-1 retest log to clarify "retest passed; channel remains tagged out under outage clearance 2026-C-0142" — removes ambiguity against the `SDS-1 Unavailable (outage)` plant_status row. Additive (no TRUNCATE), expected post-apply counts 52/15/24. Pending Raj's `supabase db push --linked` — sandbox blocked the remote push from the agent side.
- **2026-04-17** — **Phase 6 opened** (frontend overhaul, Raj-driven). After a walkthrough of the running app Raj identified 11 surface issues; the plan absorbs them as a coordinated overhaul rather than point fixes. Scope: (a) split chrome into a marketing landing shell + a ChatGPT-style app shell for KH/Generator (kills the current nav + sidebar + footer triple-stack on app pages); (b) Knowledge Hub UX — streaming-state indicator, clickable inline citations that link directly to source URLs, thread rename for all users, no auto-naming; (c) Generator UX — shadcn `Select` dropdowns, switch the turnover route to `stream: true` with a three-phase progress state (the current sequential RPC → non-streaming `max_tokens=1500` LLM is the latency Raj saw), report layout with a sticky TOC + color-coded severity callouts; (d) brand — adopt NPXai-site palette (deep navy + aurora accents) over existing Appendix G tokens, fix the low-contrast Sign-in button; (e) homepage — animated northern-lights hero (CSS/SVG, `prefers-reduced-motion` aware); (f) cross-cutting a11y + responsive + Browserbase verification. Full task list in `TODO.md` §Phase 6.
- **2026-04-17** — **Chat threads — hybrid persistence (reverses 2026-04-16 localStorage-only for signed-in users).** New model: anon users keep localStorage-only (matches Appendix H.6 "no raw-query persistence" for unauthenticated traffic and keeps the 0-RLS-surface guarantee for drive-by traffic); signed-in users get Supabase-backed `chat_threads` + `chat_messages` tables with RLS on `auth.uid() = owner_id`, plus RPCs (`list_threads`, `get_thread`, `save_message`, `rename_thread`, `delete_thread`) so route handlers stay on the anon key path. On first sign-in, localStorage threads are one-shot uploaded to the server via a dedicated RPC and then cleared locally so the account becomes source of truth. Rationale per Raj: "if I am logged in the threads should be" — cross-device persistence is table stakes for the operator UX pitch. Schema + migration will be scoped by the agent as the first Phase 6 task (Appendix K to follow once DDL lands).
- **2026-04-17** — **NPXai brand adoption for Phase 6.** Appendix G tokens stay load-bearing for contrast ratios and the four-state catalog; npxai.com's aurora-on-navy palette layers on top via new `--accent-aurora-*` tokens and a revised `--bg` gradient. No from-scratch rewrite — existing components reference tokens by name, so swapping values is a drop-in. Brand values extracted by the agent from the public npxai.com CSS; Raj confirms before commit.
- **2026-04-17** — **Phase 6 visual direction — operator-grade, not marketing-grade** (supersedes the earlier "adopt NPXai palette wholesale" framing). Reference class for the app shell (Knowledge Hub + Generator) is **Vercel Dashboard / Linear / Palantir Foundry** — instrument-panel calm, thin 1px borders, deep saturated navy surfaces, signal color only where it carries semantic meaning (`--requirement` blue + `--guidance` amber stay — they already encode concept, not decoration). JetBrains Mono used more deliberately on timestamps, REGDOC IDs, clearance codes so structured data looks structured. NPXai's aurora becomes a **marketing-hero-only motif**, rendered in our darker palette rather than their marketing teal — "classy bits with our flavour," not a clone. Zero motion on the operating pages (no aurora, no drifting gradients — an operator staring at moving chrome while reading a clearance status is bad taste). Driver: a hiring team for NPXai is evaluating whether Raj understands nuclear operators, not whether he can copy a marketing site.
- **2026-04-17** — **Light mode ships alongside dark** (reverses the 2026-04-16 Appendix G "dark-only ships" scope cut). Shadcn `next-themes` + `ThemeToggle` in the top-right of both the marketing nav and the app shell header. Full light-mode token set lives next to the dark set in `app/globals.css`. Acknowledged cost: doubles the Appendix G four-state catalog (every loading/empty/error/partial state needs a light variant) and roughly doubles the WCAG AA contrast matrix. Accepted because a hiring manager opening the demo link in daylight on a tablet is a realistic evaluation moment and reading a dark-only regulatory Q&A in bright sun is the wrong first impression.
- **2026-04-17** — **Generated-report persistence** (parallel model to the chat-thread hybrid). Supabase-backed `generated_reports` table for signed-in users with RLS on `auth.uid() = owner_id`, localStorage (capped at last N reports) for anon. Schema carries `snapshot_hash` — a hash of the plant-snapshot tuple `(station, unit, shift, plant_status.updated_at + work_orders.updated_at + shift_log.updated_at)` — so re-requests for an unchanged unit-shift return the cached report with "Last generated 12m ago, nothing has changed" + an explicit **Regenerate** escape hatch. Lives as a "Recent reports" rail in the Generator shell. Drivers: (a) wallet protection — regenerating a 1500-token report when the plant snapshot is byte-identical is pure waste; (b) premium feel — operators notice "I've seen this before" recognition; (c) parallels the chat-thread decision so the persistence story is consistent across both surfaces.
- **2026-04-17** — **Phase 6 ordering** (recommendation accepted by Raj). Sequence: **6A chrome split** → **6D light/dark token scaffold** (quick, unblocks all visual work) → **6C Generator streaming + left-rail jump + report persistence** (biggest UX-latency fix) → **6B KH citations + rename + no auto-name** → **6D brand palette swap** → **6B thread persistence RPC suite** (most code, ships late so a bug here doesn't block the other five surfaces) → **6E aurora hero on marketing only** → **6F a11y + responsive + Browserbase + green gates**. Full scope stays intact per Raj — we're ahead of schedule and the overhaul ships before outreach resumes.
- **2026-04-17** — **Unified single sidebar across all breakpoints** (supersedes the two-sidebar KH layout). The app shell previously stacked an outer app-switcher sidebar + an inner KH thread-history sidebar (`lg:`-only); mobile and tablet never saw the KH thread list at all. Collapsed into one `AppShell` sidebar: app nav (Knowledge Hub, Generator) above a divider, pathname-gated contextual rail below — KH route shows the thread list + "New thread", Generator route shows the Recent Reports list. Fills the previously-empty middle region of the sidebar and restores thread history parity on mobile/tablet via the existing `Sheet` drawer. Generator form ↔ sidebar now communicate via a small `useGeneratorStore` zustand bridge (`refreshKey` + `pendingLoad`) because `RecentReports` no longer lives inside `GeneratorForm`'s tree. `KnowledgeHubShell` stripped back to just the chat surface (inner sidebar + its localStorage collapse key retired). `ThreadSidebar` + `RecentReports` take an optional `onNavigate` that closes the mobile sheet on selection.
- **2026-05-14** — **Prompt-injection hardening** (Phase 10 — supersedes the 2026-04-16 "jailbreak prefixes logged, not blocked" stance). Red-team battery against NPX's public bot exposed the gaps; ours closes five: (1) `KNOWLEDGE_HUB_SYSTEM` rewritten — citation/phrasing answer rules kept verbatim; `PROMPT_VERSION` → `2026-05-14.3`. The rewrite must stay **answer-first** ("Your job is to answer … Default to answering") with a clearly-scoped "Security boundary — refuse ONLY these" block after: an earlier refuse-first draft regressed the hard eval 20/20→18/20 by refusing terse in-corpus queries ("turnover"). Final: hard eval 20/20, security eval 30/30. (2) User query now spotlighted in `<user_query>` with HTML-escaped body, mirroring the `<context_snippet>` treatment. (3) `sanitizeQueryText` adds NFKC normalize + zero-width strip; `JAILBREAK_PATTERNS` expanded 4→16 (incl. French — Canada is bilingual); new `decodeBase64Probe` re-scans base64 payloads; new `leetFold` lets `detectJailbreakMarkers` catch leetspeak obfuscation; new `HARD_INPUT_CEILING = 8000`. (4) Jailbreak markers now **short-circuit** to the canonical out-of-scope reply (raw + base64-decoded scan) via the cache-hit streaming path — skips embed + RPC + LLM, zero attacker feedback channel. (5) New adversarial eval harness `evals/security.jsonl` + `scripts/eval-security.ts` (`bun run evals:security`), grouped-by-category markdown table = the artifact for the demo video page. No tool surface, single-turn, RLS on chat tables — unchanged and out of scope.
- **2026-07-13** — **Phase 11 opened (Raj): Knowledge Hub Artifact mode + RAG eval framework.** (a) **Artifact mode** — a search-time toggle that, instead of the chat interface, generates a high-quality self-contained HTML explainer ("artifact") for a complex regulatory topic, NPX-themed, with inline-SVG diagrams/visuals aiding the text. Security posture decided up front: artifact renders ONLY in a sandboxed iframe (never `allow-same-origin` + `allow-scripts` together); the artifact route reuses the SAME deterministic injection guard + tier rate limits as chat; the brand shell (palette/typography CSS) is injected deterministically by our code, the LLM fills content only — no LLM-invented hex, no external fetches. (b) **RAG eval framework** — experiments comparing corpus contents vs agent answers using established metrics (RAGAS/TruLens-family: faithfulness, answer relevancy, context precision/recall + consistency), heavy structured logging, per-run cost guard (wallet protection), delivering a committed report with realistic percentages per rating category. Evals stay manual `bun run` scripts, never in the build gate.
- **2026-07-13** — **Orchestrated-delivery process adopted for Phase 11.** Role-split subagent loop (planner → executor → reviewer → fix → steward) with repo-resident, code-free specs. Coordination docs live committed under `docs/orchestration/` (role templates, backlog, token ledger, friction log, prompt changelog, manual-verification queue). PRs are per-slice; merges to `main` are safe (no CI auto-deploy — Cloudflare deploys stay manual via wrangler).
- **2026-07-13** — **Timelapse scaffolding removed.** The frontend-evolution-timelapse experiment's local, never-committed artifacts (`.timelapse.yaml`, `.timelapse/`, `.env.timelapse`, and an uncommitted `.gitignore` edit) deleted/reverted from the working tree per Raj; nothing timelapse-related ever reached git history, and the tool can regenerate its config on demand.
- **2026-07-14** — **Local Supabase adopted for dev + evals; hosted stays for the deployed demo (hybrid).** Runbook: `supabase/LOCAL.md`. The hosted free project auto-paused a 3rd time and took the Phase 11 RAG eval battery with it. The full stack now runs in Docker on the Mac mini ($0, never pauses): 15 migrations replay clean, corpus re-ingests to 1,945 chunks / 19 docs, anon-key vector search verified over PostgREST, magic-link auth works with mail caught by Inbucket. **Local cannot serve the deployed demo** — Cloudflare's edge can't reach `127.0.0.1`, and no config changes that. Rejected: Cloudflare Tunnel to the Mac mini (makes home uptime the demo's uptime and publishes default-credential Postgres/Studio from a home LAN — unacceptable for a link a hiring team opens) and full self-hosting (same uptime dependency + we'd own backups/TLS/upgrades). Decision: **local for dev/evals/ingest/schema, hosted free project + a daily keep-alive cron for the public demo** (free tier pauses after ~7 days idle; a *weekly* ping races that boundary, so ping daily). Keep-alive Worker is specified in `LOCAL.md` but **not yet built** — outstanding.
- **2026-07-14** — **Bug found + fixed by replaying migrations locally: new signups were 500ing.** `20260417131500_owner_email_allowlist.sql` did `CREATE OR REPLACE FUNCTION handle_new_user()` without restating `SET search_path = public`; Postgres *drops* config params you omit on replace, so the trigger lost its search_path (`proconfig` → NULL, while `get_user_tier` kept `{search_path=public}`). It fires as `supabase_auth_admin`, whose search_path excludes `public`, so unqualified `profiles` stopped resolving → `relation "profiles" does not exist` → `500: Database error saving new user`. Latent since 2026-04-17 and invisible in normal use because the trigger only runs for **new** users — existing accounts sign in fine. Fixed in `20260714000000_fix_handle_new_user_search_path.sql` (restates the SET, schema-qualifies `public.profiles`). **Almost certainly present on hosted too — needs `supabase db push`.**
- **2026-07-14** — **PR #12 round-2 review fix: `ALTER ROLE` alone is invisible to a running PostgREST, and the ingest's wipe-then-insert was unsafe against hosted.** Two gaps caught by review: (1) `20260714010000_service_role_statement_timeout.sql` only wrote the catalog row — PostgREST applies a role's `rolconfig` at boot/reload only, never on `SET ROLE`, so hosted's long-lived PostgREST would have kept enforcing the old 8s cap forever. Fixed by appending `NOTIFY pgrst, 'reload config';`; verified locally by changing `service_role`'s timeout via raw SQL *without* restarting local PostgREST and confirming a REST-API call ignores the new value until the NOTIFY is sent, then honors it immediately after — proves the mechanism independent of `db reset`'s "restart picks it up for free" false positive. (2) `scripts/ingest.ts` TRUNCATEd `regdoc_chunks` before inserting, non-transactionally — any mid-run failure (e.g. the timeout above) left the corpus wiped-and-partial, catastrophic against the live hosted demo. Fixed by routing inserts through a new `regdoc_chunks_staging` table + one atomic swap function (`ingest_swap_regdoc_chunks_staging`, `20260714020000_regdoc_chunks_staging_swap.sql`) that truncates-and-refills `regdoc_chunks` inside a single Postgres transaction, called only after staging's row count is verified — chosen over a simple "refuse to run against non-local URL" guard flag because that alone wouldn't stop a *deliberate* hosted re-ingest from still wiping-and-partial-ing on failure; the non-local guard (`--force` / `ALLOW_REMOTE_INGEST=1`) was added anyway as cheap defense-in-depth against running by accident. Verified the swap function's rollback-on-mismatch and success paths locally with dummy vectors (no OpenAI cost).
- **2026-07-14** — **Embedding upgrade: `text-embedding-3-small`@1536 → `text-embedding-3-large`@3072 via `halfvec` (PR #13).** Measured win on the hardened 92-Q golden set (LOCAL, real OpenAI): hit@8 95.7%→96.7%, recall@8 82.6%→86.0%, MRR 0.761→0.808. Storage stays flat (halfvec = 2 B/dim, so 3072 halfvec = 1536 float = 6144 B/row); HNSW uses `halfvec_cosine_ops` (pgvector ≥0.7; hosted verified 0.8.0). Migration `20260714030000` drops+re-adds the `embedding` column and recreates `match_regdoc_chunks(halfvec(3072))`. **Rejected** a cross-encoder reranker stage in the same investigation — it *regressed* hit-rate (honest negative). **Deploy constraint (from adversarial review):** the RPC signature change is incompatible in BOTH directions, so an in-place swap has an unavoidable short outage window regardless of merge order — merge is HELD behind a hosted runbook (pause cron → optional embedding snapshot → `db push` → re-ingest at -large via staging swap → merge → un-pause). The branch code is correct for the migrated target state; the constraint is deploy sequencing, not a code defect.
...
- **2026-07-15** — **Best-practices corpus expansion prepared (Phase 12, PR `feat/best-practices-corpus`, prod HELD).** Ingested 26 CNSC guidance REGDOCs (Crown Copyright GREEN) via a new license-manifest-driven fetcher (`scripts/fetch-best-practices.ts`; CNSC = Gatsby page-data `mdx.body` HTML; PDF path via `unpdf` validated on NRC but NRC not ingested). Local-validated: 19→45 docs, 1,945→3,394 chunks, 0 null embeddings. **IAEA excluded from ingestion** (license = excerpts-with-permission, not full-corpus storage — security/legal decision-priority #1) → 16 IAEA guides kept as reference-only manifest bibliography. **NRC deferred** — its non-CNSC ids need source-aware citation plumbing + a prompt security-boundary rewrite (currently refuses "non-Canadian regulation") + threshold recalibration, too security-sensitive to land unsupervised. **Adversarial eval-honesty review overturned an initial rosy read:** naive ingestion is a REAL one-directional retrieval regression (ksweep hit@8 96.7→91.3, **~15 questions degraded / 0 improved at any k**), driven by §1.3/front-matter boilerplate collision (26 near-identical admin sections) + the over-broad REGDOC-2.14.1 distractor — NOT mostly staleness. Mitigated by a fetcher boilerplate-drop (hit@8→93.5, −175 chunks) but a residual remains (12 degraded / 0 improved) → **recommend NOT loading prod until a supervised generation eval + possible retrieval retune** ([[feedback_component_win_can_regress_pipeline]], [[feedback_diagnostic_certainty]]). Two self-introduced defects caught by adversarial review + fixed pre-PR: (1) 10 volume-suffixed ids (`REGDOC-2.11.1-VolII`) broke citation regexes → extended with `-Vol[IVX]`; (2) `QUERY_DOC_RE` truncated volume ids to their base → boosted the WRONG volume (existing Vol I) → fixed. Lint + test:frontend/artifact/rag-eval green.

- **2026-09-23** — **Common-overview consolidation tested; prior root-cause attribution narrowed.** A local-only ablation moved 41 exact-titled *CNSC Regulatory Document Series* sections into one provenance reference (27 source wordings), reducing the expanded corpus 3,394→3,353 chunks. On 91 paired questions, hit@8 and context recall@8 were unchanged; MRR rose only 0.2 pp. §1.3 legislation and Preface sections are not uniformly duplicate and can be gold evidence, so no blanket removal. REGDOC-2.14.1 appears in some off-scope top-eight results, but simple demotion did not recover misses. No production corpus or retrieval-policy change; added stage-by-stage workbench and kept prod HELD.
...
- [x] Dev server loads `/`, `/knowledge-hub`, `/generator`, `/insights`, `/equivalency` without runtime errors. *(Verified 2026-04-17 via Browserbase ui-test — all 5 routes snapshot cleanly, console error array empty on page load for /knowledge-hub, axe clean on /knowledge-hub + /equivalency.)*
- [x] Knowledge Hub page renders `ThreadListSidebar` + `Thread`; a mock message round-trips successfully. *(Verified 2026-04-17 via Browserbase `ui-test` skill — 20/20 tests pass across 2 groups: mock streams the expected REGDOC-2.3.4 text in ≥2 snapshots, empty/double submit gated, sign-in modal opens/closes, keyboard-only composer works, mobile 375×812 shows zero horizontal overflow, console clean. Axe: 0 violations on /knowledge-hub and /equivalency after dropping the layout's wrapper `<main>` and adding `role="complementary" aria-label="Thread history"` to the sidebar. Starter `/` still has 1 `region` violation — Phase 4 replaces that page.)*
