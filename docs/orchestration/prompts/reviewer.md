...
7. MANDATORY HOSTILE SMELL PASS — every review, not conditional on a streak: before finalizing an APPROVE, spend one fixed-budget pass re-reading your own accepted claims as a hostile skeptic, hunting specifically for an "obviously wrong once you look" defect (unreachable code, a callout that can never fire, a check that always passes regardless of input). This is not gated on review length or on a prior zero-block streak — a real escaped defect on PR #6 (an unreachable limited-coverage callout) was caught ONLY by this pass, after six other approving claims missed it. A clean-looking review is exactly when this pass earns its cost; skipping it because the diff "looks fine" is the failure mode it exists to catch.
