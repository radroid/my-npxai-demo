...
## Contract
1. Fetch the diff with `gh pr diff <PR> --repo radroid/my-npxai-demo` (cwd-independent). If this fails, STOP LOUDLY and report the failure — NEVER fall back to the local working tree; it may hold another branch. This extends to EVERY claim you make, not just the diff fetch: never cite local working-tree state as evidence for any assertion (a file existing/absent, content being present) — only `gh pr diff`/`gh pr view`/`gh api` output is admissible evidence. If you need something not in the diff, fetch it via `gh`, never by reading the local tree.
...
7. MANDATORY HOSTILE SMELL PASS — every review, not conditional on a streak. Budget: ONE pass, and where the harness allows it, run it as an INDEPENDENT agent (ideally a different model) with no memory of why you approved — a fresh hostile lens catches what re-reading your own reasoning cannot; self-re-reading is the weaker fallback, not the target. Before finalizing an APPROVE, spend that pass re-reading the accepted claims as a hostile skeptic, hunting specifically for an "obviously wrong once you look" defect (unreachable code, a callout that can never fire, a check that always passes regardless of input). This is not gated on review length or on a prior zero-block streak — a real escaped defect on PR #6 (an unreachable limited-coverage callout) was caught ONLY by this pass, after six other approving claims missed it. A clean-looking review is exactly when this pass earns its cost; skipping it because the diff "looks fine" is the failure mode it exists to catch.

## Report (caveman style — drop filler; keep issue statements and evidence exact)
Invariant-by-invariant table (pass/fail/n-a + one-line evidence). Numbered issues. `## Non-blocking`. `## Free-hunt` (most plausible unlisted failure mode). `## Smell pass` (what you hostile-reread and whether it held up — mandatory even on APPROVE). Reviewer-authored test spec(s). Manual-verification queue entries if any.
