...
Executor, 2026-07-13. Slice 1.2 only; branch `feat/artifact-frontend`. No backend files touched.

**What was built** — everything in spec §Slice 1.2:

- `components/knowledge-hub/ModeToggle.tsx` — segmented Chat/Artifact control, `aria-pressed` buttons (spec's allowed alternative to radiogroup), canonical tokens only: container `bg-surface-2 border-border`, active `bg-brand text-white shadow-sm` (AppShell active-nav precedent), inactive `text-fg-muted hover:text-fg`, `focus-visible:ring-brand`. Rendered in BOTH surfaces so the control never moves on screen.
- `components/assistant-ui/thread.tsx` — `composerHeader?: ReactNode` prop, rendered between `ThreadScrollToBottom` and `Composer` inside `ViewportFooter`. Default `undefined` → `{composerHeader}` renders nothing → markup identical to the prop-less Thread. Composer/transport untouched.
- `components/knowledge-hub/KnowledgeHubShell.tsx` — mode state default `"chat"`; header hint text swaps per mode; both surfaces mounted, `hidden` class swap only (chat wrapper keeps its exact `min-h-0 flex-1 overflow-hidden` classes in chat mode).
- `hooks/use-artifact-stream.ts` — statuses `idle → retrieving → drafting → ready | error`; `res.ok` checked before any stream read (400 → `validation`, 429 → `rate_limit`, other → `server`, all with server `message` verbatim); SSE frame parsing copied from the `useGenerateStream` precedent; `meta` → drafting, `progress` → token count, `artifact` → result + ready, `error` → kind from `code`; stream-end-without-artifact → network error. AbortController aborts on new run, `stop()`, and unmount.
- `components/knowledge-hub/ArtifactWorkbench.tsx` — empty state (one-line explainer + the 4 spec starter topics, click = fill + submit), composer-styled form (same class string as the chat composer shell for visual kinship, `maxLength={2500}` soft bound, disabled while running), Generate/Stop round buttons mirroring `ComposerAction`, progress pill reusing the thinking-pill visual (`animate-breathe` dot, live `~N tokens`), viewer iframe `sandbox="allow-popups allow-popups-to-escape-sandbox"` + `srcDoc` + descriptive `title` inside `border-border rounded-xl bg-surface` chrome with query/time caption bar, Download-.html (`npx-artifact-<slug>-<yyyymmdd>.html`, Blob → download-attribute anchor, object URL revoked synchronously; no open-in-new-tab per I1.7), cached + truncated pills, `SourcesPanel data={{ chunks }}`, error card (`border-danger/40 bg-danger/10`, heading `text-danger`, message `text-fg` verbatim) with Retry; out-of-scope errors additionally re-offer the starter topics per the off-corpus edge case.

**Deviations taken (small-call autonomy):**
- **Escape abort is a window-level listener**, active only while running AND the workbench is visible (`rootRef.offsetParent !== null`). The spec put Escape on the input, but the input is disabled during a run (spec requirement) so its keydown can never fire; the visibility gate preserves the "toggle to Chat does NOT abort the hidden run" edge case (Escape pressed in chat is ignored).
- Enter (without Shift) submits the textarea — kinship with the chat composer; Shift+Enter still inserts a newline.
- Viewer iframe height is `h-[65dvh] min-h-[60dvh]` inside the scrollable column (not flex-fill): actions row + SourcesPanel below it must stay reachable; meets the spec's "min ~60dvh".
- `role="group"` dropped from ModeToggle (biome `useSemanticElements` demands `fieldset`, which carries default-style baggage); the two labelled `aria-pressed` buttons are self-describing.
- Workbench column width `max-w-[44rem]` matches the thread's `--thread-max-width` so the two surfaces line up visually.
- New-run start clears the previous artifact (progress state replaces it); an abort mid-run therefore lands on the empty state, not the stale artifact. Toggling never clears anything.

**Verification:** `bun run lint` green (`Checked 97 files. No fixes applied.`), `bun run build` green, `bun run test:artifact` green (56 checks). No live UI verification — dev server down by dispatch; exact both-themes click-through appended to docs/orchestration/manual-verification.md.

**What the reviewer should look at hard:** the `thread.tsx` diff (must be additive-only: import type, prop, one JSX line), the iframe `sandbox` string (I1.1 load-bearing), the Download path (Blob never reaches `window.open`/navigable href — I1.7), and hidden-run behavior around the mode toggle.
